...
CourseView::CourseView(Data *data)
    : _data(data),
      _exitfocus(0),
      _exitselected(0),
      _createfocus(0),
...
void CourseView::Init()
{
    ...
    _deletebox.setOrigin(sf::Vector2f(_deletebox.getGlobalBounds().width / 2,
                                      _deletebox.getGlobalBounds().height / 2));
    ...
    _exportbox.setOrigin(sf::Vector2f(_exportbox.getGlobalBounds().width / 2,
                                      _exportbox.getGlobalBounds().height / 2));
    ...
    _export.setOrigin(
        sf::Vector2f(_export.getGlobalBounds().width / 2, _export.getGlobalBounds().height / 2));
    ...
    _scorebox.setOrigin(sf::Vector2f(_scorebox.getGlobalBounds().width / 2,
                                     _scorebox.getGlobalBounds().height / 2));
    ...
    _score.setOrigin(
        sf::Vector2f(_score.getGlobalBounds().width / 2, _score.getGlobalBounds().height / 2));
    ...
    _updatebox.setOrigin(sf::Vector2f(_updatebox.getGlobalBounds().width / 2,
                                      _updatebox.getGlobalBounds().height / 2));
    ...
    while (_data->_window->pollEvent(event))
    {
        ...
        if (event.type == sf::Event::MouseMoved)
        {
            ...
            _deletefocus = (_deletebox.getGlobalBounds().contains(
                                ...
                                ? 1
                                : 0);
            _exportfocus = (_exportbox.getGlobalBounds().contains(
                                sf::Vector2f(sf::Mouse::getPosition(*_data->_window).x,
                                             sf::Mouse::getPosition(*_data->_window).y))
                                ? 1
                                : 0);
            _scorefocus  = (_scorebox.getGlobalBounds().contains(
                               sf::Vector2f(sf::Mouse::getPosition(*_data->_window).x,
                                             sf::Mouse::getPosition(*_data->_window).y))
                                ? 1
                                : 0);
            ...
            for (int i = 0; i < cur; i++)
            {
                _studentfocus[i] = (_student[i].getGlobalBounds().contains(
                                        sf::Vector2f(sf::Mouse::getPosition(*_data->_window).x,
                                                     sf::Mouse::getPosition(*_data->_window).y))
                                        ? 1
                                        : 0);
...
void CourseView::Update()
{
    ...
    for (int i = 0; i < cur; i++)
    {
        (_studentfocus[i] ? _student[i].setFillColor(sf::Color::Red)
                          : _student[i].setFillColor(sf::Color::Black));
    }
    for (int i = 0; i < cur; i++)
    {
